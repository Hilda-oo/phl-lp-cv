...
void ThermoelasticSim::solve() {
  F_.resize(0, 1);
  F_.resize(load_.size(), 1);
  for (int i = 0; i < nDof_; i++) {
    F_.coeffRef(i, 0) = load_(i) 
    #ifndef MECH_ONLY
    + Fth_(i)
    #endif
    ;
  ...
  // U_ = solver_.solve(F_);
  solver_init.solve(Fvec);
  spdlog::info("compliance Cm = {:e}", Fvec.col(0).dot(U_));
