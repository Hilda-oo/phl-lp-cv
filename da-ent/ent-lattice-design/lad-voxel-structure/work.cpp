...
class FeaturePreservedLatticeStructureProcessor {
 public:
  ...
  void GenerateLatticeSigendDistanceFieldForBoundingBox(size_t structure_type,
                                                        ...
                                                        double radius) {
    lattice_structure_sdf_ = sha::MicroStructureSDF(structure_type, structure_num,
                                                    mesh_bounding_box_, radius, sdf_sample_domain_);
 ...
 public:
  sha::MicroStructureSDF lattice_structure_sdf_;
  sha::FrameSDF shell_lattice_sdf_;
  sha::MeshSDF mesh_sdf_;
};
}  // namespace da

auto GenerateVoxelLatticeStructure(size_t num_samples_on_longest_side,
                                   size_t num_cells_along_x_axis, size_t num_cells_along_y_axis,
                                   size_t num_cells_along_z_axis, double radius_of_lattice_frame,
  ...
  auto lattice_out_path       = WorkingResultDirectoryPath() / "lattice-voxel.obj";
  auto uncut_lattice_out_path = WorkingResultDirectoryPath() / "uncut_lattice-voxel.obj";
  ...
  if (radius_of_lattice_frame > 0) {
    ...
    processor.GenerateLatticeSigendDistanceFieldForBoundingBox(
        lattice_structure_type, num_xyz_cells, radius_of_lattice_frame);
  }
