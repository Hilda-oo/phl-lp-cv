...
class FieldDrivenVoxelStructureOperator(bpy.types.Operator):
    bl_idname: str = "designauto.lattice_design_field_driven_voxel"
    bl_label: str = "场引导体素晶格设计"
    bl_options = {"REGISTER", "UNDO"}

    lattice_type: bpy.props.EnumProperty(
        items=[(f"{type}", f"Type {type}", f" Type{type}") for type in supported_types])

    lattice_radius: bpy.props.FloatProperty(name="Lattice(inner) Beam Radius", default=0.04)
    field_type: bpy.props.EnumProperty(
        items=[(f"{type}", f"{type}", f"{type}") for type in supported_field_types])

    field_coeff: bpy.props.FloatProperty(name="Coeffient for Field", default=1)   
    ...
    num_xyz_cells: bpy.props.IntVectorProperty(name="Number of cells",
                                               description="Cell numbers along x,y,z axes", default=[20, 6, 6])
    ...
    def execute(self, context):
        ...
        import dapy_lad_field_driven_voxel
        os.chdir(executable_path)
        lattice_mesh = dapy_lad_field_driven_voxel.GenerateVoxelLatticeStructureByField(
            self.num_samples, self.num_xyz_cells[0],self.num_xyz_cells[1],self.num_xyz_cells[2], 
            int(self.lattice_type), self.field_type, self.field_coeff, self.lattice_radius,
            self.shell_radius, self.sharp_angle, working_object.name == self.last_object and self.last_num_samples == int(self.num_samples))
        ...
        new_mesh = bpy.data.meshes.new("mesh-" + working_object.name + "-field-driven-voxel-lattice")
        new_mesh.from_pydata(lattice_mesh.mat_coordinates.tolist(), [], lattice_mesh.mat_faces.tolist())
        new_mesh.update()
        new_object = bpy.data.objects.new(working_object.name + "-field-driven-voxel-lattice", new_mesh)
        found = False
