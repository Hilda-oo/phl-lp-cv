...
//     Restricted Voronoi Diagram (intersection between a mesh and a 3D Voronoi
//     diagram) Restricted Delaunay Triangulation (dual of a Restricted Voronoi
//     Diagram) Clipped Voronoi Diagram (intersection between the interior of a
//     mesh and a 3D Voronoi diagram)
//
//===================================================================================================

// Quick reference:
//
// Mesh: stores a Piecewise Linear Complex (+ functions to read it from a file
// in .obj format) Delaunay: abstract interface to Delaunay triangulation (+
// Delaunay_CGAL: implementation with CGAL) RestrictedVoronoiDiagram: Given a
// Mesh and a Delaunay, computes the intersection between a Voronoi diagram and
// a Mesh. ClippedVoronoiDiagram: computes the intersection between a Voronoi
// diagram and the interior of a closed Mesh
//
// RestrictedVoronoiDiagram and ClippedVoronoiDiagram are accessed through the
// for_each_triangle(do_it) function, that does the actual computation, and
// calls the user-provided function do_it(i,j,k,l) for each integration simplex.
// Note that do_it can be a function or an object that overloards operator().
// This mechanism can be used for both computing F-Lp and displaying the clipped
// Voronoi cells as in figures 4,5,6 in the paper.
...
void load_pts_xyz(const std::string &filename, std::vector<vec3> &pts,
                  int num_seeds) {
  pts.clear();
  std::ifstream in_stream(filename.c_str());
  if (!in_stream) {
...
class SaveClippedVDFacets {
 public:
  SaveClippedVDFacets(Delaunay *delaunay, std::ostream &out, double shrink)
      : delaunay_(delaunay), out_(out), shrink_(shrink), cur_(1) {
    out << "# attribute chart facet integer" << std::endl;
  }
  void operator()(unsigned int i, int j, const vec3 &p1, const vec3 &p2,
                  const vec3 &p3) const {
 ...
 */
void save_clippedVD(ClippedVoronoiDiagram &CVD, const std::string &filename,
                    double shrink) {
  std::ofstream out(filename.c_str());
  if (!out) {
    std::cerr << "could not open file." << std::endl;
...
void test_combinatorics(const std::string &mesh_filename,
...
class MemorizeIndices {
 public:
  MemorizeIndices(std::vector<int> &I_in, std::vector<vec3> &C_in)
      : I(I_in), C(C_in) {
    I.resize(0);
    C.resize(0);
  }

  void operator()(unsigned int i, int j, const VertexEdge &v1,
                  const VertexEdge &v2, const VertexEdge &v3) const {
    I.push_back(i);
...
class MemorizeIndicesAndFacets {
 public:
  MemorizeIndicesAndFacets(const RestrictedVoronoiDiagram &RVD_in,
                           std::vector<int> &I_in, std::vector<vec3> &C_in,
                           std::vector<int> &F_in)
  ...
  void operator()(unsigned int i, const VertexEdge &v1, const VertexEdge &v2,
                  const VertexEdge &v3) const {
    I.push_back(i);
    I.push_back(v1.sym[2]);
    I.push_back(v1.sym[1]);
    I.push_back(v1.sym[0]);
 ...
 *
 */
void get_combinatorics(Mesh *M, const std::vector<vec3> &pts,
                       std::vector<int> &I, std::vector<vec3> &C,
                       std::vector<int> &F, bool volume) {
  Delaunay *delaunay = Delaunay::create("CGAL");
  delaunay->set_vertices(pts);
  if (volume) {
    ClippedVoronoiDiagram CVD(delaunay, M);
 ...
 */
void compute_F_g(Mesh *m, const std::vector<vec3> &pts, unsigned int p,
                 bool volume) {
  std::cerr << "nb pts = " << pts.size() << "   nb facets = " << m->nb_facets()
            << std::endl;
  std::vector<int> I;
  std::vector<vec3> C;
  std::vector<int> F;
  ...
  for (unsigned int i = 0; i < M.size(); i++) {
    ...
    //      the current integration simplex (and access an array of per-facet
    //      anisotropy).
  ...
  std::cerr << (volume ? "volume " : "surface ") << "F_L" << p << ":"
            << "f=" << std::scientific << f << "  g=" << gnorm << std::endl;
}

void test_algebra(const std::string &mesh_filename,
                  const std::string &pts_filename) {
  Mesh M;
  unsigned int nb_borders = M.load(mesh_filename);
  std::vector<vec3> pts;
...
namespace Geex {
void LpLlyod(const std::string &mesh_filename, const std::string &pts_filename,
             const int P, const int num_iterations) {
  Mesh mesh;
  unsigned int nb_borders = mesh.load(mesh_filename);
  std::vector<vec3> pts;
  load_pts_xyz(pts_filename, pts, 5000);

  const int num_points = pts.size();

  std::cerr << "nb pts = " << pts.size()
            << "   nb facets = " << mesh.nb_facets() << std::endl;
  std::vector<vec3> variables = pts;

  for (int iteration_idx = 0; iteration_idx < num_iterations; ++iteration_idx) {
    std::vector<int> I;
    std::vector<vec3> C;
    std::vector<int> F;
    get_combinatorics(&mesh, variables, I, C, F, true);
    unsigned int nb_integration_simplices = (unsigned int)I.size() / 10;
    std::cout << nb_integration_simplices << " " << C.size() << std::endl;
    std::cout << mesh.nb_facets() << std::endl;
    // exit(5);
    // break;
    std::vector<mat3> M(nb_integration_simplices);
    std::set<int> iset;
    for (unsigned int i = 0; i < M.size(); i++) {
      M[i].load_identity();
      vec3 seed = variables[I[i * 10]];
      // double l = seed.length();
      double p = ::sqrt((seed.x * seed.x) + (seed.y * seed.y));
    ...
    std::vector<plane3> Q(mesh.nb_facets());
    for (unsigned int i = 0; i < mesh.nb_facets(); i++) {
      Q[i] = mesh.facet_plane(i);
    }

    std::vector<double> gradients(num_points * 3);

    double f = compute_F_Lp(true, P, &mesh, I, C, variables, Q, M, gradients);
    Eigen::MatrixXd g(num_points, 3);
    for (int idx = 0; idx < num_points; ++idx) {
      g.row(idx) << gradients[3 * idx + 0], gradients[3 * idx + 1],
          gradients[3 * idx + 2];
    }
    double norm = g.norm();
    double step = 5e-3;
    for (int idx = 0; idx < num_points; ++idx) {
      variables[idx].x -= step * gradients[3 * idx + 0] / norm;
      variables[idx].y -= step * gradients[3 * idx + 1] / norm;
      variables[idx].z -= step * gradients[3 * idx + 2] / norm;
    }
    std::cout << "#iter: " << iteration_idx + 1 << "\t f: " << f
              << "\t mean: " << g.colwise().mean() << "\t norm: " << g.norm()
              << std::endl;
  }
  {
    Delaunay *delaunay = Delaunay::create("CGAL");
    RestrictedVoronoiDiagram RVD(delaunay, &mesh);
    ClippedVoronoiDiagram CVD(delaunay, &mesh);

    delaunay->set_vertices(variables);
    save_RVD(RVD, "lloyd_rvd.obj");
    save_RDT(RVD, "lloyd_rdt.obj");

    if (nb_borders == 0) {
      save_clippedVD(CVD, "lloyd_cvd.obj", 0.7);
    }
    delete delaunay;
  }
}
...
int main(int argc, char **argv) {
  //   if (argc != 3) {
  //     std::cerr << "usage  : " << argv[0] << " mesh_filename pts_filename"
  //               << std::endl;
  //     std::cerr << "example: " << argv[0]
  //               << " data/three_holes.obj data/three_holes.pts" << std::endl;
  //     return -1;
  //   }
  // std::string mesh_filename = "../data/three_holes.obj";
  // std::string pts_filename = "../data/three_holes.pts";

  std::string mesh_filename = "../bunny/bunny.obj";
  std::string pts_filename = "../bunny/seeds.txt";
  Geex::LpLlyod(mesh_filename, pts_filename, 6, 20);

  //   std::cerr << "============= geometry->combinatorics test =========="
  //             << std::endl;
  //   Geex::test_combinatorics(mesh_filename, pts_filename);
  //   std::cerr << "============= combinatorics->algebra test  =========="
  //             << std::endl;
  //   std::cerr << "(note: expect large values for f and g)" << std::endl;
  //   Geex::test_algebra(mesh_filename, pts_filename);
  return 0;
}
