...
// For debuggin'
int at(Eigen::MatrixXi &M, const int i, const int j) { return M(i, j); }

// mesh-in, mesh-out wrapper
void meshfix(const Eigen::MatrixXd &Vin, const Eigen::MatrixXi &Fin, Eigen::MatrixXd &Vout,
             Eigen::MatrixXi &Fout) {
  /////////////////////////////////////////////////////////////////////////
...
Energies implemented: None, DispStep, DispInitial, Volume, SurfARAP, VolARAP
  ...
  for (int i = 0; i < k; i++) {
    ...
    // with CGAL decimation (regular or adaptive)
      ...
      // first check if last charcater of argv[i+2] is r. If it is, drop the 'r' and adaptive =
      // false
    ...
    if (!flow_fine_inside_coarse(V, F, V_coarse, F_coarse, A_qv, H)) {
      cout << "Flow failed to take fine mesh inside coarse mesh after 1000 iterations. Quitting"
           << endl;
      return 0;
    }
    cout << "Success!" << endl;
    ...
    if ((asprintf(&suffix, "_%d.obj", i + 1) != -1) &&
        (asprintf(&filename, "%s%s", argv[argc - 1], suffix) != -1)) {
 ...
 */
namespace cage {
std::string EnergyTypeToStr(const EnergyType &type) {
  switch (type) {
    case EnergyType::None:
      return "None";
    case EnergyType::DispStep:
      return "DispStep";
    case EnergyType::DispInitial:
      return "DispInitial";
    case EnergyType::Volume:
      return "Volume";
    case EnergyType::SurfARAP:
      return "SurfARAP";
    case EnergyType::VolARAP:
      return "VolARAP";
    default:
      return "None";
  }
...
auto nested_cages(const Mesh &mesh0, int quad_order, const std::vector<LevelInfo> &level_info,
                  EnergyType energy_inflation, EnergyType energy_final) -> std::vector<Mesh> {
  using namespace igl::copyleft::cgal;
  // number of layers
  int k = level_info.size();
  std::vector<Mesh> mesh_levels;
  ...
  Surface_mesh M;
  // convert to CGAL format
  mesh_to_polyhedron(mesh0.V, mesh0.F, M);

  // output input mesh as level output_0.off
  // mesh_levels.push_back(mesh0);
  ...
  // Loop over levels
  for (int i = 0; i < k; i++) {
    // the input decimation is computed by decimating the previous layer
    // with CGAL decimation (regular or adaptive)
    // specified number of faces for this cage
    L[i]     = level_info[i].num_faces;
    adaptive = level_info[i].adaptive;
    std::cout << "L" << i + 1 << "/" << k << " #" << level_info[i].num_faces << " r" << adaptive
              << std::endl;
    // value to pass to the decimator
    float ratio = (1. * L[i]) / (1. * M.size_of_facets());
    // the previously computed will be decimated
    ...
    // to define the integral at _every_ step - the metric is fixed)
    VectorXd area_0;
    doublearea(fine_mesh.V, fine_mesh.F, area_0);
    area_0 = 0.5 * area_0;
    // Precompute matrix that convert gradients at quadrature points to gradients at mesh vertices
    SparseMatrix<double> A_qv;
    gradQ_to_gradV(fine_mesh.V, fine_mesh.F, area_0, quad_order, A_qv);
    // Flow M inside M_hat and save the result to a stack M of flow meshes
    stack<MatrixXd> H;
    cout << "Flowing M" << i << " inside M" << i + 1 << "..." << endl;

    if (!flow_fine_inside_coarse(fine_mesh.V, fine_mesh.F, V_coarse, F_coarse, A_qv, H)) {
      cout << "Flow failed to take fine mesh inside coarse mesh after 1000 iterations. Quitting"
           << endl;
      exit(3);
    }
    ...
    MatrixXd C;
    cout << "Reinflating M" << i << ", pushing M" << i + 1 << "..." << endl;
    reinflate(H, fine_mesh.F, V_coarse, F_coarse, EnergyTypeToStr(energy_inflation).data(),
              EnergyTypeToStr(energy_final).data(), C);
    cout << "Success!" << endl;
    ...
    // sanity check: cage should never intersect input decimation
    intersect_other(C, F_coarse, fine_mesh.V, fine_mesh.F, true, IF);
    if (IF.rows() > 0) {
      cout << i + 1 << "-th output cage intersect previous cage. ERROR! Quitting...  " << endl;
      exit(3);
    }
    ...
    mesh_to_polyhedron(C, F_coarse, M);
    fine_mesh.V = C;
    fine_mesh.F = F_coarse;
    mesh_levels.push_back(fine_mesh);
    // back to adaptive decimation (standard)
    adaptive = true;
  }
  return mesh_levels;
